create table member_search_hit
(
    ...
    constraint fk_followers_members10
        foreign key (searching_member_id) references members (member_id),
    constraint fk_followers_members20
        foreign key (searched_member_id) references members (member_id)
);

create index fk_followers_members1_idx
    on member_search_hit (searching_member_id);

create index fk_followers_members2_idx
    on member_search_hit (searched_member_id);
